    ...
    <section class="w-full column p-10 g-10">
        <div class="bg-light g-10 br-10 column p-10">
            <strong class="desc row align-center g-5 c-primary">
             ...
             <hr class="bg-primary">
             <form action="{{ url('admins/post/edit/gift/code/process') }}" onsubmit="PostRequest(event,this,Completed)" method="POST" class="w-full column g-10">
               {{-- CODE ID --}}
               ...
               <input type="hidden" name="_token" value="{{ @csrf_token() }}" class="inp input">
                ...
                <div class="column g-5 w-full">
                    ...
                    <div style="border:1px solid rgba(255,255,255,0.1);background:rgba(255,255,255,0.05);" class="w-full br-5 cont bg">
                        <input type="number" value="{{ $data->value }}" name="value" placeholder="E.G &#8358;500.00" class="inp input required w-full border-none br-5 bg-transparent">
                    </div>
                </div>
                    {{-- NEW INPUT --}}
                  <div class="column g-5 w-full">
                    ...
                    <div style="border:1px solid rgba(255,255,255,0.1);background:rgba(255,255,255,0.05);" class="w-full br-5 cont bg">
                        <input type="number" name="length" placeholder="E.G 16" value="{{ strlen($data->code) }}" class="inp input required w-full border-none br-5 bg-transparent">
                    </div>
                </div>
                    {{-- NEW INPUT --}}
                 <div class="column g-5 w-full">
                    ...
                    <div style="border:1px solid rgba(255,255,255,0.1);background:rgba(255,255,255,0.05);" class="w-full br-5 cont bg">
                        <input type="number" name="limit" placeholder="E.G 1000" value="{{ $data->limit }}" class="inp input required w-full border-none br-5 bg-transparent">
                    </div>
                </div>
                    {{-- SUBMIT BUTTON --}}
